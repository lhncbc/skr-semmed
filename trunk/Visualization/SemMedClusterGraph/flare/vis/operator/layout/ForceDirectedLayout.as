	...
	public class ForceDirectedLayout extends Layout
	{
		...
		protected function update(d:DataSprite):Boolean
		{
			...
			if (!p.fixed) {
				...
				_moved = _moved || (Math.abs(_t.$(d).x - p.x) > _epsilon) || (Math.abs(_t.$(d).y - p.y) > _epsilon);
		...
		{			
			...
			if (p == null) {
				...
				p.fixed = d.fixed || !d.visible;
			} else {
				with (p) { x = d.x; y = d.y; fixed = d.fixed || !d.visible; }
			}
			p.tag = _gen;
			return true;
		}
