package flare.vis.data
{
	...
	public class Data extends EventDispatcher
	{
		...
		/**
		 * Dongwook Shin, 01/05/2010
		 * Adds a node to this data collection if the node is not in the collection.
		 * @param d either a data tuple or NodeSprite object. If the input is
		 *  a non-null data tuple, this will become the new node's
		 *  <code>data</code> property. If the input is a NodeSprite, it will
		 *  be directly added to the collection.
		 * @return the newly added NodeSprite
		 */
		public function addUniqueNode(cluster:String, d:Object=null):NodeSprite
		{
			var ns:NodeSprite = NodeSprite(d is NodeSprite ? d : newNode(d));
			var obj:Object = _nodes.getNode(ns)
			// var aNode:NodeSprite = NodeSprite(_nodes.getNode(ns));

			if(obj != null) {
              // Alert.show('node is already in the list in addUniqueNode()', 'Alert Box', mx.controls.Alert.OK);

               var aNode:NodeSprite = NodeSprite(obj);
               var msg1:String = 'Cluster = ' + cluster;
                var vcluster:String = aNode.data.cluster + "_" + cluster;
                // Alert.show(vcluster);
                aNode.data.cluster = vcluster;
				return null;
			} else {
			 	// Alert.show('--- node is not in the list in addUniqueNode() ---', 'Alert Box', mx.controls.Alert.OK);
				_nodes.add(ns);
				fireEvent(DataEvent.DATA_ADDED, ns);
				return ns;
			}
		...
		{
			// 02/24/2011 DW Temporarily commented out
		...
		{
			// DW 02/24/2011 Temporarily commented out
		 ...
		 * @return the newly added EdgeSprite
 		 */
		public function addUniqueEdgeFor(cluster:String, source:NodeSprite, target:NodeSprite,
			directed:Object=null, data:Object=null):EdgeSprite
		{
			// DW 02/24/2011 Temporarily commented out
			// if (!_nodes.contains(source) || !_nodes.contains(target)) {
			...
			var d:Boolean = directed==null ? directedEdges : Boolean(directed);
			var e:EdgeSprite = newEdge(source, target, d, data);
			var obj:Object = _edges.getEdge(e)

			if(obj != null) {
              // Alert.show('edge is already in the list in addUniqueEdge()', 'Alert Box', mx.controls.Alert.OK);
               var anEdge:EdgeSprite = EdgeSprite(obj);
               var msg1:String = source.data.name + "-" + data.predicate + "-" + target.data.name;
               // Alert.show(anEdge.data.cluster);
               // Alert.show(msg1);
                var vcluster:String = anEdge.data.cluster + "_" + cluster;
               // Alert.show(vcluster);
                anEdge.data.cluster = vcluster;

                // _nodes.add(aNode);
				return null;
			} else {
			 	// Alert.show('--- edge is not in the list in addUniqueEdge() ---', 'Alert Box', mx.controls.Alert.OK);
			 	// Alert.show(data.cluster);
			 	var msg1:String = source.data.name + "-" + data.predicate + "-" + target.data.name;
			 	// Alert.show(msg1);
				source.addOutEdge(e);
				target.addInEdge(e);
