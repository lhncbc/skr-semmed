	...
	public class RK4Integrator implements IIntegrator
	{
		...
		public function integrate(sim:Simulation, dt:Number):void {
			...
			for (i=0; i<N; ++i) {
				...
				a = dt / p.mass;
				xp[i] = p.x;     yp[i] = p.y;
				xk[i] = cut(dt*p.vx); yk[i] = cut(dt*p.vy); 
				xl[i] =  cut(a*p.fx); yl[i] =  cut(a*p.fy);
				if (!p.fixed) {
					p.x += .5 * xk[i]; 
					p.y += .5 * yk[i];
				}
			...
			for (i=0, ii=N; i<N; ++i, ++ii) {
				p = particles[i] as Particle;
				a = dt / p.mass;
				xk[ii] = cut(dt*(p.vx + .5*xl[ii-N]));
				yk[ii] = cut(dt*(p.vy + .5*yl[ii-N]));
				xl[ii] =  cut(a*p.fx);
				yl[ii] =  cut(a*p.fy);
				if (!p.fixed) {
					p.x = xp[i] + .5*xk[ii];
			...
			for (i=0; i<N; ++i, ++ii) {
				...
				if (!p.fixed) {
					...
					p.vx += cut((xl[i] + 2*(xl[i+N] + xl[ii-N]) + xl[ii]) / 6);
					p.vy += cut((yl[i] + 2*(yl[i+N] + yl[ii-N]) + yl[ii]) / 6);
				} else {
					p.vx = p.vy = 0;
				}
			}
		...
		private function cut(d:Number):Number {
			if (d > MAX)
				return MAX;
			if (d < -MAX)
				return -MAX;
