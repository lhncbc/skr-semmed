...
package semmed {
	...
	public class RadialLayout extends Layout{
		...
		public override function operate(t:Transitioner=null):void
		{
			...
			// visualization.data.sort("fillColor",2);
		...
		protected function layout(node:Node,root:Boolean):void {
			node.visited = true;
			...
			/* if (node.neighbors.length > 15)
				additionalSpace = 100;
			if (node.neighbors.length > 30)
				additionalSpace = 200;
			if (node.neighbors.length > 50)
				additionalSpace = 300;
			if (node.neighbors.length > 70)
				additionalSpace = 400; */

			if (node.neighbors.length > 10 && node.neighbors.length <= 20 )
				additionalSpace = 100;
			else if (node.neighbors.length > 20 && node.neighbors.length <= 30 )
				additionalSpace = 200;
			else if (node.neighbors.length > 30 && node.neighbors.length <= 50)
				additionalSpace = 300;
			else if (node.neighbors.length > 50)
				additionalSpace = 400;

			for each(n in node.neighbors)
				if (!n.visited) {
					n.x = node.x + (n.width+additionalSpace) * Math.cos(currentAngle);
					if(Math.cos(currentAngle) < 0.1 && Math.cos(currentAngle) > -0.1)
						n.y = node.y + (n.width + 25 + additionalSpace) * Math.sin(currentAngle);
					else
