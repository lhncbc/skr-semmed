package semmed{
	...
	// public class GraphVisualizer extends Sprite
	{
        ...
        public function setRoot(n:NodeSprite):void{
        	...
        	(os.getOperatorAt(1) as CircleLayout).layoutRoot = n;
			(os.getOperatorAt(2) as RadialLayout).layoutRoot = n;
			rootNode = n;
        ...
        public function loadData(alreadyRendered:String):void
        {
			...
			// Alert.show('step 7 in loadData()');
			vis.update();
			// Alert.show('step 8 in loadData');
			addChild(vis);
...
//			new DragControl(vis);
        ...
        public function filter(predicates:Array,semtypes:Array):void{
	        ...
	        // for each (var v:NodeSprite in allNodes)
	        //	dataF.removeNode( v);
        	// Alert.show("Removed edges");
        	for each (var e:EdgeSprite in allEdges) {
        		// var edge:String = e.source.data.name + "-" + e.data.predicate + "-" + e.target.data.name;
        		// Alert.show(edge);
        		dataF.removeEdge(e);
			 }
			// dataF.clearEdges();

			// Alert.show("Added edges");
        	for each (var x:EdgeSprite in allEdges){
				...
				if (isContained(predicates, x.data.predicate)){
        			var p:String = x.source.data.name + "-" + x.data.predicate + "-" + x.target.data.name;
        			// Alert.show(p);
        			dataF.addEdge(x);
         ...
         public function filterCluster(clusters:Array):void{
        	//var l:DataList = new DataList();
        	...
        	for each (var x:NodeSprite in allNodes){
				// 07/17/2008, Dongwook Shin, SemType is ignored for the time being
				var clname = x.data.cluster;
				// Alert.show(x.data.name, 'Alert Box', mx.controls.Alert.OK);
				// Alert.show(clname, 'Alert Box', mx.controls.Alert.OK);
				// if (isContained(clusters, x.data.cluster)){
				if (isContainedCluster(clusters, clname)){
        			dataF.addNode(x);
        			// Alert.show('added', 'Alert Box', mx.controls.Alert.OK);

        		}
        	}

        	for each (var y:EdgeSprite in allEdges){
				// 07/17/2008, Dongwook Shin, SemType is ignored for the time being
				var clname = y.data.cluster;
				// if (isContained(clusters, y.data.cluster)){
				if (isContainedCluster(clusters, clname)){
        			dataF.addEdge(y);
        			y.source.visible = true;
        			y.target.visible = true;
        		}
        	}
        	vis.update();
        }
        ...
        public function isContainedCluster(x:Array,y:String):Boolean{
        	// Alert.show(y, 'Alert Box', mx.controls.Alert.OK);
        	for each(var z:String in x)
        		if (y.indexOf(z) >= 0) {
        			// Alert.show(z, 'Alert Box', mx.controls.Alert.OK);
        			return true;
        		}
        	return false;
        }
