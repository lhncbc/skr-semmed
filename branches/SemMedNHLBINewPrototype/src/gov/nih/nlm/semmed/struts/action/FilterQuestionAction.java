...
public class FilterQuestionAction extends LookupDispatchAction  {
	...
	public ActionForward select(
		...
		Map questionNumberMapping = ((Map) ctx.getAttribute("questionNumberMapping"));
		LabelValueBean qe = (LabelValueBean) questionMapping.get(new Integer(qnum));
		question = qe.getLabel();
		String questionValue = qe.getValue();
		LabelValueBean qne = (LabelValueBean) questionNumberMapping.get(new Integer(qnum));
		String questionNumber = qne.getLabel();
		chosenQuestion = questionNumber + ". " + question;
		...
		Predicate pred = getRules(ruleString).get(questionValue);
		...
		if(session.getAttribute("relevantQuestionrCitations") != null) // remove previous relevant results for the selected questions
			session.removeAttribute("relevantQuestionrCitations");
		if(session.getAttribute("relevantQuestionnrCitations") != null) // remove previous relevant results for the selected questions
			session.removeAttribute("relevantQuestionnrCitations");
		if(session.getAttribute("relevantQuestionrPredications") != null) // remove previous relevant results for the selected questions
			session.removeAttribute("relevantQuestionrPredications");
		if(session.getAttribute("relevantQuestionnrPredications") != null) // remove previous relevant results for the selected questions
