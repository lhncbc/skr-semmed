			...
			if (relevant!=null){
				try{
					...
					while (citer.hasNext()) {
						...
						sw.append("<citation data=\""+p.getId()+"\" impact=\"" + String.format("%2.3f", p.getImpactFactor()) + "\" quality=\"" + new Double(p.getQualityEvidence()).intValue() + "\" label=\""+stringToHTMLString(p.getTitleText())+"\"/>");
					}
				}catch(Exception e){
