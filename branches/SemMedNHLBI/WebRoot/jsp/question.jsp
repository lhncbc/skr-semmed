	...
	<div id="content">
		<html:form action="Question"  method="post" enctype="multipart/form-data">
			<logic:messagesPresent>
			   ...
			   <table cellpadding="4">
				<logic:present name="displayedCitationsMedline" scope="session">
					<tr>
						<td>
				...
				<logic:present name="displayedCitationsEssie" scope="session">
					<tr>
						<td>
							<logic:present name="essiequery" scope="session">
									(<b>Essie Query:</b>
								<bean:write name="essiequery" property="term" scope="session" />
									<bean:define id="source" name="essiequery" property="source"
									scope="session" type="java.lang.String" />,
									<b>Year:</b>
									<%=session.getAttribute("essieyear")%>)
							</logic:present>
						</td>
					</tr>
				</logic:present>
			 ...
			 <table>
				...
				<logic:present name="relevantQuestionrCitations" scope="session">
					<tr>
						<td>
							<html:submit property="method">
								<bean:message key="question.button.sortrcitationif" />
							</html:submit>
						</td>
						<td>
							<html:submit property="method">
								<bean:message key="question.button.sortnrcitationif" />
							</html:submit>
						</td>
					</tr>
					<tr>
						<td>
							<html:submit property="method">
								<bean:message key="question.button.sortrcitationqe" />
							</html:submit>
						</td>
						<td>
							<html:submit property="method">
								<bean:message key="question.button.sortnrcitationqe" />
							</html:submit>
						</td>
					</tr>
					<tr>
						<logic:present name="orgRelevantQuestionrCitations" scope="session">
						<td>
							<html:submit property="method">
								<bean:message key="question.button.returnorgrcitation" />
							</html:submit>
						</td>
						</logic:present>
						<logic:present name="orgRelevantQuestionnrCitations" scope="session">
						<td>
							<html:submit property="method">
								<bean:message key="question.button.returnorgnrcitation" />
							</html:submit>
						</td>
						</logic:present>
					</tr>
			...
			<logic:present name="relevantQuestionrCitations" scope="session">
				...
				<logic:present role="SemMedTester">
					<html:form action="Question" method="post">
						<div style="float:right">
							<input type="text" name="citNum" />
							<html:submit property="method">
								<bean:message key="question.button.findAcitation" />
							</html:submit>
						</div>
					</html:form>
				</logic:present>
					...
					<c:if test="${count>0}">
						...
						<table id="pubmed" rules="rows">
							...
							%>
								<tr><th>Citations</th>
								<logic:present role="SemMedTester">
								<c:if test="${rankingsource == 1}">
								<th>Impact Factor</th>
								</c:if>
								<c:if test="${rankingsource == 2}">
