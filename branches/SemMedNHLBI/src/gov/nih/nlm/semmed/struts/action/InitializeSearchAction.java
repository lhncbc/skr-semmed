...
public class InitializeSearchAction extends Action {
	...
	public ActionForward execute(
		...
		if (session.getAttribute("query") == null) {
		    Query qm = new Query("","medline");
		    qm.setDefaultValues();
		    Query qe = new Query("","essie");
		    qe.setDefaultValues();

		    session.setAttribute("query",qm);
		    session.setAttribute("essiequery",qe);
		...
		log.debug("In InitializeSearchAction(): About to succeed:");
