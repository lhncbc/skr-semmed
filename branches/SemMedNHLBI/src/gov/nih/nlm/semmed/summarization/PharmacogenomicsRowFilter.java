 ...
 *
 */
public class PharmacogenomicsRowFilter implements SingleRowFilter {

	private static PharmacogenomicsRowFilter myInstance = new PharmacogenomicsRowFilter();

	private PharmacogenomicsRowFilter() {
	}

	public static PharmacogenomicsRowFilter getInstance() {
		return myInstance;
	}
	...
	public boolean filter(String predicate, List<String> subjSemtype,
		...
		if (predicate.endsWith(Constants.INTERACTS_WITH)
			...
			return Summarizer.intersects(Constants.SUBSTANCE_SEMGROUP,
					subjSemtype)
					&& Summarizer.intersects(Constants.SUBSTANCE_SEMGROUP,
							objSemtype);
		...
		if (predicate.endsWith(Constants.PART_OF))
			return Summarizer.intersects(Constants.SUBSTANCE_SEMGROUP,
					subjSemtype)
					&& Summarizer
							.intersects(Constants.ORG_SEMGROUP, objSemtype);
