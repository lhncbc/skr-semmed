...
public class NHLBINoveltyRowFilter implements SingleRowFilter {
	private static NHLBINoveltyRowFilter myInstance = new NHLBINoveltyRowFilter();

	private NHLBINoveltyRowFilter(){}

	public static NHLBINoveltyRowFilter getInstance(){
		return myInstance;
	}
...
//		 New NHLBI requirements from Marcelo, 10/27/2008
		if (predicate.endsWith(Constants.PREVENTS))
			return Summarizer.intersects(Constants.NHLBI_TREAT_SEMGROUP,subjSemtype) &&
			Summarizer.intersects(Constants.NHLBI_DISORDER_SEMGROUP,objSemtype);
		...
		if (predicate.endsWith(Constants.COEXISTS_WITH)) // IS case is caught at the bottom
