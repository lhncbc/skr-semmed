...
public class VisualizeAction extends LookupDispatchAction {
	...
	protected Map<String, String> getKeyMethodMap() {
		...
		map.put("visualize.button.uploadAndVisualizeR", "uploadAndVisualizeR");
		map.put("visualize.button.uploadAndVisualizeZ", "uploadAndVisualizeZ");
		return map;
	}
	 ...
	 * @return ActionForward
	 */
	public ActionForward uploadAndVisualizeR(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws UploadException {
		VisualizeForm VisualizeForm = (VisualizeForm) form;
		HttpSession session = request.getSession();
		FormFile file = VisualizeForm.getUploadGraphFileR();
		log.debug("In uploadAndVisualizeR()" + request.getContentType());
		log.debug("Request type: " + request.getContentType());
		log.debug("File type: " + file.getContentType());
		try {
			InputStream graphis   = file.getInputStream();
			// DocumentBuilderFactory docBuilderFactory = DocumentBuilderFactory.newInstance();
			SAXBuilder builder = new SAXBuilder();
			Document doc = builder.build(graphis);
		    ...
		    String newGraphString = graphString.replaceFirst("xmlns:=\"\"", "");
			// log.debug("graph String : " + newGraphString);
			UUID key = GraphServlet.addGraph(request, newGraphString, null, null);
			session.removeAttribute("cluster");
			session.setAttribute("key", key.toString());
			return mapping.findForward("success");
		} catch (Exception e) {
	 ...
	 * @return ActionForward
	 */
	public ActionForward uploadAndVisualizeZ(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws UploadException {
		VisualizeForm VisualizeForm = (VisualizeForm) form;
		HttpSession session = request.getSession();
		FormFile file = VisualizeForm.getUploadGraphFileZ();
		log.debug("In uploadAndVisualizeZ()" + request.getContentType());
		log.debug("Request type: " + request.getContentType());
		log.debug("File type: " + file.getContentType());
		try {
			InputStream graphis   = file.getInputStream();
