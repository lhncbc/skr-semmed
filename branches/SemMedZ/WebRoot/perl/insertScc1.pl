...
$delete_scc1_sth = $dbh->prepare_cached("DELETE FROM scc1") or die "Couldn't prepare statement: " . $dbh->errstr;
$insert_scc1_sth = $dbh->prepare_cached('insert ignore into scc1 (PID,SID,PNUMBER, PMID,predicate,s_cui,s_name,s_type,s_novel,o_cui,o_name,o_type,o_novel)
